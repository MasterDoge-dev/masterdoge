...
int nAnonymizeMDOGEAmount = 20000;
...
#else
    const char* pszModule = "masterdoge";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MasterDoge
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MasterDoge
    // Mac: ~/Library/Application Support/MasterDoge
    // Unix: ~/.masterdoge
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MasterDoge";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "masterdoge.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "masterdoged.pid"));
